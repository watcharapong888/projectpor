...
if (@$_SESSION['user_name'] == null || @$_SESSION['user_name'] == '') {
  ...
  <body>
    ...
    if (isset($_POST['home_id']) != null && isset($_POST['home_no']) != null && isset($_POST['swine']) != null) {
      $stmt = $conn->prepare("SELECT id_home FROM address WHERE id_home = :home_id");
      $stmt->bindParam(':home_id', $_POST['home_id'], PDO::PARAM_STR);
      $stmt->execute();

      if ($stmt->rowCount() >= 1) {
        $card_id_use = 'รหัสบ้านซ้ำ';
      } else {
        $card_id_use = 'รหัสบ้านไม่ซ้ำ';
      }
      if ($card_id_use == 'รหัสบ้านไม่ซ้ำ') {
            ...
            <div class="container mt-3">
              <div class="row">
                <div class="col">
                  ...
                  <input type="text" class="form-control" id="home_id" name="home_id" maxlength="11"
                    placeholder="ระบุตัวเลขไม่เกิน 11 ตัว" pattern="[0-9]{11}" title="กรุณากรอกตัวเลข 11 หลัก" required>
                ...
                if ($result != null) {
                  ...
                  foreach ($result as $row) {
                    ...
                    <tr>
                      ...
                      <td>
                        <?php
                        $id_home = str_pad($row['id_home'], 11, '0', STR_PAD_LEFT); // เติมเลข 0 จนได้ 11 ตัว
                        $formatted_id_home = substr_replace($id_home, '-', 4, 0); // ใส่ขีดที่ตำแหน่งที่ 5
                        $formatted_id_home = substr_replace($formatted_id_home, '-', 11, 0); // ใส่ขีดที่ตำแหน่งที่ 11
                        echo $formatted_id_home;
                        ?>
                      </td>
